...
1. Rather than specifying where to start the new row for devices, just select how many devices you want to see in 1 row.
2. You can select the sort by name or type of device. (Type of device will sort by icon)
...
git clone https://github.com/bpcode/MMM-NetworkScanner.git
````
...
| `deviceInOneRow:` | 5 | `optional` Number of devices in one row if showDeviceColums is true. |
| `sortBy:` | `type` | `optional` Sort the devices by type or name. |
